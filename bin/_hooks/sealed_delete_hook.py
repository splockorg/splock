...
def main() -> int:
    ...
    # test override) lives in `bin._hooks.sealed_paths_file` — this module used
    # to hardcode the `.claude/hooks/` layout, which does not exist in this
    # fork, so it always took the `FileNotFoundError -> allow` branch and failed
    # OPEN on deletion of sealed state.
