...
CALL2_SYSTEM_PATCH: Final[str] = (
    ...
    "  - a scalar `field` is a plan-root field name (title, problem_statement, "
    "tier, ...) OR the one allowlisted nested path "
    "`conceptual_architecture.overview`. Use that dotted form to amend the "
    "architecture overview prose; it is the ONLY dotted address accepted. Never "
    "address a keyed-collection entry by path (no `success_criteria.0.criterion`) "
    "— those have their own op_kind.\n"
