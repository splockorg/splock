...
| mysql-mcp-guard — read-only gate on the MCP lane: PreToolUse statement filter on every `mysql`-prefixed server (`mcp__mysql.*`) + per-server `SHOW GRANTS` credential probe; /qna+/recon spawn gate (`probe`, `--all` sweep); write-capable exit 51, unverifiable exit 52 with a named reason (fail closed §4.7); late-bound lanes (Secrets Manager / Vault launchers) declare `SPLOCK_MYSQL_MCP_CREDENTIAL_COMMAND` per server so a correct setup is verifiable rather than permanently refused, and a lane that declares no credential of its own refuses (`unattributed`) instead of grading whichever `MYSQL_USER` is in scope; `SPLOCK_MYSQL_MCP_GUARD` halt/warn/off | `bin/mysql-mcp-guard`, `bin/_mysql_mcp_guard/`, `hooks/mysql-mcp-guard.sh`, `bin/_hooks/mysql_mcp_guard_hook.py`; `tests/test_mysql_mcp_guard.py` |
...
| **Outstanding ledger + lazy-dump cap on the append (§9)** | `bin/_route_issue/outstanding.py`, `bin/lazy-dump-check`, `hooks/lazy-dump-cap.sh`, `docs/outstanding_issues.md` (OI-1…OI-6); `tests/acceptance/test_acceptance_D_lazy_dump_check_cap.py` |
...
| **splock's own ledger is inside its own seal** (§4.4, §4.15) | **resolved 2026-08-05 (`OI-6`).** The ledger moved from the repo root to `docs/outstanding_issues.md`, which is what `hooks/sealed_paths.txt` seals, what `bin/_route_issue/outstanding.py` writes, and what `hooks/lazy-dump-cap.sh` caps. splock's own ledger is now CLI-only on the same terms as an adopter's. The entry's earlier "not fixable by an agent" framing was wrong for the option actually taken — moving the file touches no sealed path |
