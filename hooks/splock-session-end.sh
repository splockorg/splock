#!/usr/bin/env bash
# .claude/hooks/splock-session-end.sh — SessionEnd hook (Phase C).
#
# Marks the session row closed + flips any still-open subagent rows to
# 'done'. Replaces the 240-min TTL as the primary close trigger.
#
...
if [ -z "$CLAUDE_SESSION_ID" ]; then
    "$REPO_ROOT/bin/hook-log" splock-session-end ok "no_session_id" >/dev/null 2>&1 || true
    exit 0
fi
...
if [ "$RC" -ne 0 ]; then
    ...
    "$REPO_ROOT/bin/hook-log" splock-session-end error "$DETAIL" >/dev/null 2>&1 || true
else
    "$REPO_ROOT/bin/hook-log" splock-session-end ok "ok" >/dev/null 2>&1 || true
fi
