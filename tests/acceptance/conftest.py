...
The source-of-truth inventory this suite was written against lives in the origin
repo's plan history and is not carried here; `tests/acceptance/README.md`
describes the §A–§M blocks.

This conftest provides the shared fixtures, the per-session Markdown findings
report flag, and the fork's artifact-gated skips.
...
# NOTE: no module-level `pytestmark` here. A conftest is not a test module, so a
# mark set on it applies to nothing — and evaluating `pytest.mark.acceptance` at
# conftest IMPORT time happens before this file's own `pytest_configure` has
# registered the marker, which emitted a PytestUnknownMarkWarning on every run.
# The 112 test modules carry `pytestmark = pytest.mark.acceptance` themselves,
# which is where it actually selects (`pytest -m acceptance`).
...
def _write_acceptance_report(path: Path) -> None:
    ...
    lines = [
        ...
        "--acceptance-report=...`.",
        "",
        "Block reference: `tests/acceptance/README.md`",
        "",
        "---",
        "",
        "## Summary",
